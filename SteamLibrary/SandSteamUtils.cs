...
namespace SteamLibrary
{
    public class SandSteamUtils
    {
        ...
        public static WindowHandle GetSandSteamLoginWindow(string sandName)
        {
            return TopLevelWindowUtils.FindWindow(wh =>
                wh.GetClassName().Equals($"Sandbox:{sandName}:vguiPopupWindow") &&
                (wh.GetWindowText().Contains("Steam") &&
                 wh.GetWindowText().Contains(sandName) &&
                 !wh.GetWindowText().Contains("-") &&
                 !wh.GetWindowText().Contains("—") &&
                 wh.GetWindowText().Length > 5));
        }

        public static WindowHandle GetSandSteamGuardWindow(string sandName)
        {
        ...
        public static WindowHandle GetSandSteamWarningWindow(string sandName)
        {
            return TopLevelWindowUtils.FindWindow(wh =>
                wh.GetClassName().Equals($"Sandbox:{sandName}:vguiPopupWindow") &&
                (wh.GetWindowText().StartsWith($"[#] [{sandName}] Steam -  [#]") ||
                 wh.GetWindowText().StartsWith($"[#] [{sandName}] Steam —  [#]")));
        }

        public static Process WaitForSandSteamProcess(WindowHandle windowHandle)
        {
